...
- Natural order of the entries. +
  ...
  This is like `String.compareTo` on the qualified names,
  except that the current implementation adds chars to
  distinguish the different segments of the name and
  those cause a slight deviation from the ordering in
  the specification.
